...
void Snake::UpdateBody(SDL_Point &current_head_cell, SDL_Point &prev_head_cell) {
  ...
  if ((!growing) && (!shrinking)) {         // if not growing and not shrinking
    // Remove the tail from the vector.
    body.erase(body.begin());
  } else if (growing) {              // if growing = true
    growing = false;    // set growing back to false
    size++;            // increase size of snake
  }
  else {                   // if shrinking = true
    shrinking = false;    // set shrinking back to false
    size--;            // decrease size of snake
    if (body.size() < 2){
      alive = false;
      return;
    }
